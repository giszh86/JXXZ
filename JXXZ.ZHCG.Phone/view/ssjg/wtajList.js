...
require(['imm', 'mui', 'app', 'dataGet', 'pic', 'pop', 'picker', 'common', 'Vue'], function(_, mui, app, dataGet, pic, pop, picker, common, Vue) {
	mui.init({
		pullRefresh: {
	...
	function pullupRefresh() {
		...
		var url = app.webApi + 'api/phone/GetIllegalList?' + param;
		console.log(url);
		mui.ajax({
			url: url,
			success: function(data) {
				...
				for(var i = 0; i < data.Items.length; i++) {
					...
					if(item.imgUrl){
						item.imgUrl=app.imageApi+"GetPictureFile.ashx?PathClass=WTCarOriginalPath&PicPath="+item.imgUrl	
					}else{
						item.imgUrl="../../image/report/wtaj.png"
					}
