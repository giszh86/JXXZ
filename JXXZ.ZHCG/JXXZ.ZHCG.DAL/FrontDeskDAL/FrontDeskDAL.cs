                    ...
                    foreach (string item in lx)
                    {
                        switch (item)
                        {
                            case "ry":
                                ...
                                break;
                            case "cl":
                                string clsql = string.Format(@"select carid as id,'cl' as type,x84,y84 from qw_carlastpositions where (6371.004*ACOS(SIN({0}/180*PI())*SIN(Y84/180*PI())+COS({0}/180*PI())*COS(Y84/180*PI())*COS(({1}-X84)/180*PI())))<{2} and carid<>0  and positiontime>=str_to_date('{3}','%Y/%m/%d %H:%i:%s')", y84, x84, bj, time);
                                IEnumerable<FrontDeskModel> clqueryable = db.Database.SqlQuery<FrontDeskModel>(clsql);
                                if (clqueryable.Count() != 0)
                                    list.AddRange(clqueryable);
                                break;
                            case "sj":
                    ...
                    foreach (string item in lx)
                    {
                        switch (item)
                        {
                            case "ry":
                                ...
                                break;
                            case "cl":
                                string clsql = string.Format(@"select carid as id,'cl' as type,x84,y84 from qw_carlastpositions where carid<>0  and positiontime>=str_to_date('{0}','%Y/%m/%d %H:%i:%s')", time);
                                IEnumerable<FrontDeskModel> clqueryable = db.Database.SqlQuery<FrontDeskModel>(clsql);
                                if (clqueryable.Count() != 0)
                                    list.AddRange(clqueryable);
                                break;
                    ...
                    foreach (string item in lx)
                    {
                        switch (item)
                        {
                            case "ry":
                                ...
                                break;
                            case "cl":
                                string clsql = string.Format(@"select carid as id,'cl' as type,x84,y84 from qw_carlastpositions where carid<>0  and positiontime>=str_to_date('{0}','%Y/%m/%d %H:%i:%s')  LIMIT {1}, {2}", time, start, limit);
                                IEnumerable<FrontDeskModel> clqueryable = db.Database.SqlQuery<FrontDeskModel>(clsql);
                                clqueryable = clqueryable.Where(a => (a.x84 <= X1 && a.y84 <= Y1)
                                        && (a.x84 <= X1 && a.y84 >= Y2)
                                        && (a.x84 >= X2 && a.y84 <= Y1)
                                        && (a.x84 >= X2 && a.y84 >= Y2));
                                if (clqueryable.Count() != 0)
