...
and t.syncrowguid='{0}'", syncrowguid);
                List<Audit_project_wModel> sqlQueryable = orcldb.Database.SqlQuery<Audit_project_wModel>(sql).ToList();
                IEnumerable<Audit_project_wModel> Queryable = from a in sqlQueryable
                                                              join c_join in mysqldb.xzsp_locate on a.pviguid equals c_join.xzspid into ctmp
                        ...
                        switch (filter.property)
                        {
                            case "row_id":
                                if (!string.IsNullOrEmpty(value))
                                    Queryable = Queryable.Where(t => t.row_id.ToString().Contains(value));
                                break;
                            case "processversioninstancename":
                                if (!string.IsNullOrEmpty(value))
                                    Queryable = Queryable.Where(t => t.processversioninstancename != null && t.processversioninstancename.Contains(value));
                                break;
                            case "applyername":
                                if (!string.IsNullOrEmpty(value))
                                {
...
order by t.banjiedate desc";
        ...
        public int assignMidTeam(Audit_project_wModel model)
        {
            using (Entities db = new Entities())
            {
                ...
                {
                    saveLocation(model.pviguid, model.location);
                }
...
ORDER BY t.id asc", unitid);
        ...
        public approvalDetail GetFlowDetail(string pviguid, int userid)
        {
            using (Entities db = new Entities())
            {
                ...
                //获取定位信息
                if (getLocation(pviguid) != null)
                {
                    model.location = getLocation(pviguid);
                }
        ...
        /// <returns></returns>
        public int saveLocation(string pviguid,string geography)
        {
            using (Entities db = new Entities())
            { 
                xzsp_disposes dispose = db.xzsp_disposes.FirstOrDefault(t =>t.xzspid == pviguid);
                if (dispose != null)
                {
                    xzsp_locate dblocate = db.xzsp_locate.FirstOrDefault(t => t.xzspid == pviguid);
                    if (dblocate == null)
                    {
                        xzsp_locate locate = new xzsp_locate();
                        locate.xzspid = pviguid;
                        locate.location = geography;
                        db.xzsp_locate.Add(locate);
                    }
                    else
                    {
                        dblocate.location = geography;
                    }
        ...
        #region 获取定位信息
        public string getLocation(string pviguid)
        {
            using (Entities db = new Entities())
            {
                xzsp_locate locate = db.xzsp_locate.FirstOrDefault(t => t.xzspid == pviguid);
                if (locate != null)
                {
