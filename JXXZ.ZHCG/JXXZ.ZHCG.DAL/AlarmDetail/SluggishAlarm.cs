...
namespace JXXZ.ZHCG.DAL.AlarmDetail
{
    ...
    public class CarSluggishAlarm
    {
        ...
        /// <param name="family"></param>
        /// <returns></returns>
        public IQueryable<qw_cartasks> GetAllCarTaskAreas()
        {
            Entities db = new Entities();
            return db.qw_cartasks;
        }
        ...
        /// </summary>
        /// <returns></returns>
        public List<qw_carhistorypositions> Get1000PatrolDataToTemporary(int carid)
        {
            Entities db = new Entities();
            List<qw_carhistorypositions> List = db.qw_carhistorypositions.Where(a => a.isanalyze == 0 && a.carid == carid).OrderBy(t => t.positiontime).Take(1000).ToList();
            Console.WriteLine("读取1000条数据");
        ...
        /// <returns></returns>
        public IQueryable<qw_carlastpositions> GetAllCarLatestPositions(decimal carid)
        {
            Entities db = new Entities();
            IQueryable<qw_carlastpositions> us = db.qw_carlastpositions.Where(t => t.carid == carid);
            return us;
        }
        ...
        /// <param name="id"></param>
        /// <returns></returns>
        public int UpdateISANALYZE(int carid, DateTime date)
        {
            Entities db = new Entities();
            qw_carhistorypositions _model = db.qw_carhistorypositions.FirstOrDefault(t => t.carid == carid && t.positiontime == date);
            if (_model != null)
            {
                _model.isanalyze = 1;
            }
        ...
        /// </summary>
        /// <returns></returns>
        public IQueryable<qw_alarmcarmemorylocationdatas> GetAllList()
        {
            Entities db = new Entities();
            IQueryable<qw_alarmcarmemorylocationdatas> list = db.qw_alarmcarmemorylocationdatas.OrderByDescending(t => t.gpstime);
            return list;
        }
