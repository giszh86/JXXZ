Ext.define('TianZun.controller.qwmanager.CarDuty', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.carDuty',
    ...
    onAddOK: function (button, e) {
        ...
        formData['carnum'] = this.getView().down('hidden[name=carnum]').getValue();
            ...
            PostAjax({
                ...
                complete: function (jqXHR, textStatus, errorThrown) {
                    ...
                    if (textStatus == "success") {
                        ...
                        Ext.Ajax.request({
                            url: configs.WebApi + 'api/CarDuty/GetCarDutyView?timeParams=' + timeParams + '&unitid=' + unitid,
                            method: 'get',
                            scope: this,
    ...
    onDeleteOK: function (button, e) {
        ...
        Ext.Msg.confirm("提示", "您确定要执行删除操作吗？", function (btn) {
            if (btn == "yes") {
                ...
                GetAjax({
                    url: "/api/CarDuty/DeleteCarTask?cartaskid=" + formData["cartaskid"],
                    complete: function (jqXHR, textStatus, errorThrown) {
                        Ext.Msg.close();
                        if (textStatus == "success") {                            
                            var middleteamid = view.down('form').down('combo[name=middleteam]').getValue();
                            var groupid = view.down('form').down('combo[name=group]').getValue();
                            var unitid = middleteamid == null ? $.cookie('UNIT_ID') : groupid == null ? middleteamid : groupid;
