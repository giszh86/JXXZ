Ext.define('TianZun.controller.citizenservice.CitizenEvent', {
    ...
    EventMonitorList: function (button, e) {
        ...
        this.getView().down('eventAdd').add(win);
        win.show();
    },
    ...
    onAddOK: function (button, e) {
        ...
        $.ajax({
            ...
            async: false,
            success: function (response) {
                if (response == true && formData['oldeventid'] == formData['eventid']) {
                    ajaxbool = true;
                }
            }
        });
        ...
        if (formData["zptype"] == '2017021410240002')//中队派遣
        {
            ...
            //上报派遣到乡镇
            formData['pqxzid'] = formData['middleteam'];
    ...
    onDispatch: function (obj, record) {
            ...
            complete: function (jqXHR, textStatus, errorThrown) {
                if (textStatus == "success") {
                    ...
                    if (!jsonstr) {
                        ...
                        var win = Ext.create((record.get('workflowtype') == '2017021410240003' && record.get('xzid') == null) ? 'widget.eventInfoQuick' : 'widget.eventInfo', { record: record, type: 2 });
                        thisview.add(win);
                        win.show();
                    }
                } else {
        ...
        //if (ValueInArr($.cookie('ROLE_IDS').split(','), 2, 1)) {
        //    GetAjax({
        //        url: "api/CitizenEvent/EventIsHandle?wfsaid=" + record.get('wfsaid'),
        //        complete: function (jqXHR, textStatus, errorThrown) {
        //            if (textStatus == "success") {
    ...
    onDispatchOK: function (button, e) {
        ...
        if (formData["wfdid"] == '2017021410240001')//指挥中心派遣
        {
            ...
            //指挥中心派遣到乡镇
            if (formData['assigntype'] == '2017021410240002') {
                if (record.get('pqxzid') != null) {
                    var issametown = ValueInArr(record.get('pqxzid').split(','), formData['middleteam'], 1);
                    if (issametown)
                        formData['pqxzid'] = record.get('pqxzid');
                    else
                        formData['pqxzid'] = record.get('pqxzid') + ',' + formData['middleteam'];
                }else
                    formData['pqxzid'] = formData['middleteam'];
            }
        ...
        else if (formData["wfdid"] == '2017021410240003')//队员处理
        {
            ...
            formData["nextperson"] = (iswhichpagestr == 'eventInfoQuick' && ValueInArr($.cookie('ROLE_IDS').split(','), 18, 1)) ? usernq : (iswhichpagestr == 'eventInfoQuick' && ValueInArr($.cookie('ROLE_IDS').split(','), 3, 1)) ? '0' : formData["backperson"];
    ...
    changeShowInfo: function (obj, record) {
        var me = obj.up('eventAdd') != null ? obj.up('eventAdd') : obj.up('eventInfo') != null ? obj.up('eventInfo') : obj.up('eventInfoQuick');
    ...
    //短信改变显示人员及号码(处理)
    clChangeShowInfo: function (obj, record) {
        var me = obj.up('eventAdd') != null ? obj.up('eventAdd') : obj.up('eventInfo') != null ? obj.up('eventInfo') : obj.up('eventInfoQuick');
        var radiovalue = me.down('radiogroup[name=assigntype]').down('radio[checked=true]').inputValue;
        Ext.Ajax.request({
            url: 'api/User/GetUserById?userid=' + record.get('ID'),
        ...
        //var ksclnamestr = radiovalue == '2017021410240003' ? me.down('combo[name=middleperson]').rawValue + ',' + me.down('combo[name=chargeman]').rawValue : '';
        //ksclnamestr = (ksclnamestr.substr(0, 1) == ',' || ksclnamestr.substr(-1, 1) == ',') ? ksclnamestr.replace(',', '') : ksclnamestr;
        var usernamestr = (radiovalue == '2017021410240002' || radiovalue == '2017021410240007') ? me.down('combo[name=middleperson]').rawValue : radiovalue == '2017021410240008' ? me.down('combo[name=middleteam]').rawValue : ksclnamestr;
