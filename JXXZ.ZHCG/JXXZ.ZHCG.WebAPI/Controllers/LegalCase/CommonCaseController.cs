...
namespace JXXZ.ZHCG.WebAPI.Controllers.LegalCase
{
    public class CommonCaseController : ApiController
    {
        ...
        /// <returns></returns>
        [HttpGet]
        public Paging<List<CaseList>> GetAllCaseList(string filter, int start, int limit, int? XZID = null)
        {
            List<Filter> filters = JsonConvert.DeserializeObject<List<Filter>>(filter);
            Case_CasesBLL bll = new Case_CasesBLL();
            return bll.GetAllCaseList(filters, start, limit, "2017030613400001", XZID);
        }
