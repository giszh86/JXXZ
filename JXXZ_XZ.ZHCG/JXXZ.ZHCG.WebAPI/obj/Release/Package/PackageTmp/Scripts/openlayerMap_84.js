(function(window, ol) {
    function OpenlayerMap(options) {
        ...
        //绘点标注图层
        var strPoint = '{ "type": "Feature","geometry": {"type": "Point", "coordinates": ' + op.valueExtra + '},"properties": {"prop0": "value0"}}';
        self.markerSource = new ol.source.Vector({
            features: (op.modelExtra == 1 || (op.modelExtra == 0 && op.showModelExtra==1)) ? (new ol.format.GeoJSON()).readFeatures(strPoint) : null,
        });
        ...
        self.markerLayer = new ol.layer.Vector({
            ...
            style: new ol.style.Style({ //样式
                image: new ol.style.Icon(({
                    ...
                    src: 'Images/images/定位.png',
                }))
            })
        });

        //绘线标注图层
        var strLine = '{ "type": "Feature","geometry": {"type": "LineString","coordinates": [' + op.valueExtra + ']},"properties": {"prop0": "value0","prop1": 0.0}}';
        self.lineSource = new ol.source.Vector({
            features: (op.modelExtra == 2 || (op.modelExtra == 0 && op.showModelExtra == 2)) ? (new ol.format.GeoJSON()).readFeatures(strLine) : null
        });
        ...
        //绘面标注图层
        var strPolygon = '{ "type": "Feature","geometry": {"type": "Polygon","coordinates": [[ ' + op.valueExtra + ' ]]},"properties": {"prop0": "value0","prop1": {"this": "that"}}}';
        self.polygonSource = new ol.source.Vector({
            features: (op.modelExtra == 3 || (op.modelExtra == 0 && op.showModelExtra == 3)) ? (new ol.format.GeoJSON()).readFeatures(strPolygon) : null
        });
        ...
        self.map = new ol.Map({
            ...
            layers: [
				...
				}),self.markerLayer,self.lineLayer,self.polygonLayer,//self.circleLayer,
            ],
	...
	OpenlayerMap.prototype.trackBack = function(lines, options) {
		...
		function TrackBack(lines, options) {
			...
			self._imagePath = options.imagePath || 'Images/images/定位.png';
