...
INNER JOIN sm_sources ss on ss.sourceid=sm.sourceid
        ...
        public bool IsCitizenRepeat(string eventid)
        {
            using (Entities db = new Entities())
            {
                ...
                sm_citizenservices model = db.sm_citizenservices.FirstOrDefault(t => t.eventid == eventid && t.createtime.Value.Year == dt.Year && t.createtime.Value.Month == dt.Month && t.createtime.Value.Day == dt.Day);
                if (model != null)
                    return true;
                else
                    return false;
